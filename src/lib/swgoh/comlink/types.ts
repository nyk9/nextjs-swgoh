...
// -------------------------------------------------------
// Comlink /guild レスポンス型
// -------------------------------------------------------

/** /guild レスポンスに含まれるメンバー情報 */
export interface ComlinkGuildMember {
  /** プレイヤーID（/player の playerId パラメータ用） */
  playerId: string;
  /** GP（ギルドレスポンス上の値） */
  galacticPower: number;
}

/** /guild レスポンスの profile オブジェクト */
...
export interface GuildUnitAggregation {
  ...
  /** 所持メンバー数 */
  memberCount: number;
  /** レリックレベル別の人数分布（例: {5: 10, 6: 7, 7: 3}） */
  byRelicLevel: Record<number, number>;
  /** 平均レリックレベル（所持者のみ） */
  averageRelicLevel: number;
  /** 最大レリックレベル */
 ...
 * ギルド全体の集計データ
 */
export interface GuildAggregation {
  /** ギルド名 */
  guildName: string;
  /** メンバー数（データ取得成功分） */
  memberCount: number;
  ...
  /** ユニット別の集計（unitId → 集計データ） */
  units: Map<string, GuildUnitAggregation>;
  /** ギルド総GP */
  totalGalacticPower: number;
}
